-- FINAL ORDER DETAILS - NEW FUNCTION NAME
...
-- New function with different name
CREATE OR REPLACE FUNCTION get_order_data(p_order_id text, p_batch_id text DEFAULT '')
RETURNS json AS $$
DECLARE
    result json;
    order_uuid uuid;
...
GRANT EXECUTE ON FUNCTION get_order_data(text, text) TO anon, authenticated;

-- DONE! Now use in Zapier:
-- URL: https://aqcmmfeimioxvcpwpafr.supabase.co/rest/v1/rpc/get_order_data
